...
var actions = require("./ext/actions.js");
...
var init = function (options, customExtensions) {
    ...
    if (options && (options.template || options.data)) {
        if (options.data) {
            ...
            var actionsData = actions(data.metadata, options.emailProcessorBackend);
            var extensions = typeof customExtensions !== 'undefined' ? customExtensions : [];
            extensions.push(actionsData);

            start(options, undefined, data.metadata, data.content, extensions);
        } else {
